...
const Sidebar: React.FC<Props> = ({ currentView, setView, isMobileOpen, toggleMobile }) => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-y-0 left-0 z-30 w-72 bg-slate-900 text-white transform transition-transform duration-300 ease-out 
      ...
      >
        ...
        <div className="px-6 py-6 border-b border-slate-800/50 bg-slate-900/50">
          ...
          {currentUser?.role === 'CITIZEN' && (
            <div className="mt-4 bg-slate-800/80 rounded-lg p-3 flex justify-between items-center border border-slate-700/50">
              ...
              <span className="text-sm font-bold text-amber-400 drop-shadow-sm">
                {currentUser.points}
              </span>
            </div>
          )}
        ...
        <div className="p-4 border-t border-slate-800/50 bg-slate-900">
          <button
            onClick={logout}
            className="w-full flex items-center justify-center gap-2.5 px-4 py-3 text-sm font-semibold text-red-400 
            hover:bg-red-950/30 hover:text-red-300 rounded-xl transition-colors 
